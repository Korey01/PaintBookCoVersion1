...
export default function FindPainters() {
  useEffect(() => {
    document.title = "Find A Painter/Decorator | PaintBook";
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return painters.filter((p) => {
      ...
      const inType =
        !type || p.skills.some((s) => s.toLowerCase().includes(type));
      ...
      const inAvail =
        !available || (p.availability?.includes("This week") ?? false);
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === "rating_desc") arr.sort((a, b) => b.rating - a.rating);
      ...
      arr.sort((a, b) => firstNum(a.priceRange) - firstNum(b.priceRange));
    else if (sortBy === "price_desc")
      arr.sort((a, b) => firstNum(b.priceRange) - firstNum(a.priceRange));
    else if (sortBy === "reviews_desc")
      arr.sort((a, b) => b.reviews - a.reviews);
    return arr;
  }, [filtered, sortBy]);
  // Distance fallback when no exact location match
  const { nearestSorted, distancesMap, usingFallback, nearestLabel } =
    useMemo(() => {
      const locRaw = params.get("location") || "";
      const loc = locRaw.trim();
      if (!loc)
        return {
          nearestSorted: sorted,
          distancesMap: new Map<string, number>(),
          usingFallback: false,
          nearestLabel: "",
        };
      ...
      const candidates = painters.filter((p) => {
        const rateLow = parseInt(
          p.priceRange.replace(/[^0-9]/g, "").slice(0, 2),
        );
        const inPrice = rateLow >= price[0] && rateLow <= price[1];
        const inRating = p.rating >= minRating;
        const inTier = tier === "any" || p.tier === tier;
      ...
      const withDist = [...candidates].sort(
        (a, b) =>
          (map.get(a.id) ?? Number.POSITIVE_INFINITY) -
          (map.get(b.id) ?? Number.POSITIVE_INFINITY),
      );

      return {
        nearestSorted: withDist,
        distancesMap: map,
        usingFallback: true,
        nearestLabel: loc,
      };
  ...
  const paged = useMemo(
    () => nearestSorted.slice((page - 1) * perPage, page * perPage),
    [nearestSorted, page],
  );
    ...
    <div className="container mx-auto px-4 py-10 space-y-6">
      <div className="rounded-xl border bg-card/70 p-4 text-sm backdrop-blur">
        <p className="font-semibold">
          Trusted painters. Escrow-protected bookings.
        </p>
        ...
        <section>
          ...
          <form
            ...
            onSubmit={(e) => {
              ...
              if (queryLocation) {
                q.set("location", queryLocation);
              } else {
                q.delete("location");
              }
              if (queryType && queryType !== "any") {
          ...
          >
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-md border px-5 py-2 text-sm font-medium shadow-md hover:bg-accent/30"
            >
              Post a job
            </button>
          </form>
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {paged.map((p) => (
              <PainterCard
                key={p.id}
                painter={p}
                distanceKm={distancesMap.get(p.id)}
              />
            ))}
            {nearestSorted.length === 0 && (
              <p className="col-span-full text-sm text-muted-foreground">
                No painters match your filters. Try adjusting them.
              </p>
            )}
          </div>
          {nearestSorted.length > perPage && (
            <div className="mt-6 flex items-center justify-center gap-3">
              ...
              <button
                ...
                onClick={() =>
                  setPage(
                    Math.min(Math.ceil(filtered.length / perPage), page + 1),
                  )
                }
              >
                Next
