...
import type { CreateDisputeRequest, DisputeResponse, ApiResponse } from "@shared/types";
...
router.post("/", authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    const { jobId, reason, description, evidence }: CreateDisputeRequest = req.body;
    ...
    const otherId = job.customerId === req.userId ? job.painterId : job.customerId;
 ...
 * Get dispute details
 */
router.get("/:disputeId", authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Check authorization
    if (dispute.initiatedBy !== req.userId && dispute.otherId !== req.userId) {
 ...
 */
router.post("/:disputeId/evidence", authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    const { disputeId } = req.params;
    ...
    if (dispute.status !== "open") {
      res.status(400).json({
        success: false,
        error: "Cannot submit evidence for disputes that are already resolved",
      });
      return;
    }
 ...
 */
router.put("/:disputeId", authMiddleware, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    const { disputeId } = req.params;
    const { status, resolution, customerRefundAmount, painterCompensationAmount } = req.body;
...
function formatDisputeResponse(dispute: any): DisputeResponse {
  return {
    ...
    evidence: dispute.evidence ? dispute.evidence.map((e: any) => ({
      id: e.id,
