 ...
 */
router.get("/:painterId/reputation", authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
router.get("/:painterId", authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    const { painterId } = req.params;
