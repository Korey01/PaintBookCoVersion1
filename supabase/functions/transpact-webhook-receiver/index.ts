...
async function sendMismatchAlert(
  ...
  if (!apiKey || !ADMIN_EMAIL) return;
  await fetch("https://api.sendgrid.com/v3/mail/send", {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      personalizations: [{ to: [{ email: ADMIN_EMAIL }], subject: `[ALERT] Transpact amount mismatch — tx ${txId}` }],
      from: { email: ADMIN_EMAIL, name: "PaintBookCo Alerts" },
      content: [{
        type: "text/plain",
